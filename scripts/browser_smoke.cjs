...
(async () => {
  ...
  await page.getByText('โรงรถของฉัน', { exact: true }).waitFor();
  console.log('PASS: refresh preserves the current tab session');
  const savedState = await context.storageState({ indexedDB: true });
  const freshContext = await browser.newContext({ storageState: savedState, viewport: { width: 1280, height: 900 } });
  const freshPage = await freshContext.newPage();
  await freshPage.goto('http://localhost:50000/garage');
  await freshPage.waitForURL('**/login');
  await freshContext.close();
  await page.getByText('โรงรถของฉัน', { exact: true }).waitFor();
  console.log('PASS: a fresh browser context starts signed out');
