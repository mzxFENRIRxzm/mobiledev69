...
สถานะปัจจุบัน 25 กันยายน 2026: สมัครสมาชิกบน Django โดยคง Flutter OIDC Authorization Code + PKCE เดิม เลือกสมาชิกทั่วไปหรือผู้ให้บริการ พร้อมเบอร์โทร รูปร้าน และพิกัด อีเมลไม่บังคับและบัญชีใหม่เข้าใช้ได้ทันที ดู [การเปลี่ยน flow บัญชี](phase-4-email-accounts.md)

## การใช้งาน

1. เปิด `http://localhost:50000/login` แล้วกดเข้าสู่ระบบ THE_X
2. ที่หน้า Django เลือก **สมัครสมาชิกใหม่**
3. กรอกชื่อผู้ใช้ เบอร์โทร รหัสผ่านและยืนยันรหัสผ่าน แล้วเลือกประเภทสมาชิก อีเมลเป็นข้อมูลเสริม
...
5. ระบบพากลับหน้า Login โดยไม่ส่งอีเมล เข้าสู่ระบบด้วยบัญชีที่สร้างและยอมรับ Consent สมาชิกทั่วไปกลับโรงรถ ผู้ให้บริการเข้าหน้างานช่างและเปิด **ร้านบริการ** เพื่อดูร้านของตน รูป และลิงก์แผนที่
...
- บัญชีใหม่ active ทันทีหลังสมัคร ไม่มีอีเมลยืนยัน; การสมัครสาธารณะต้องเพิ่มมาตรการป้องกันบัญชีปลอม/สแปมก่อนเปิดจริง
- หน้าแก้โปรไฟล์ลูกค้า/ช่างและการยืนยันอีเมลใหม่ยังอยู่ ดู [การใช้งานและสิทธิ์](phase-4-profile.md) การรีเซ็ตด้วย username ของ Docker demo อยู่ใน [งานบัญชี](phase-4-email-accounts.md) การจองช่วงเวลาและ production deployment อยู่นอกส่วนนี้
- หากกรอกอีเมล ระบบยังตรวจค่าซ้ำและรูปแบบก่อนบันทึก; ข้อความอีเมลซ้ำอาจช่วยให้เดาว่าอีเมลถูกใช้แล้ว ต้องทบทวน UX และ rate limiting ก่อนเปิดสาธารณะ

## ตั้งค่าและ migration

`PUBLIC_SIGNUP_ENABLED` ค่าเริ่มต้นตาม `DEBUG`; สามารถตั้ง `false` เพื่อซ่อนลิงก์และปิดทั้ง GET/POST หน้าสมัครได้ Production ควรปิดไว้จนมี rate limiting, การจัดการร้าน/บัญชีปลอม และการทบทวนความเสี่ยงจากการสมัครโดยไม่ยืนยันตัวตน `FRONTEND_LOGIN_URL` ใช้กำหนดหน้ากลับ Flutter เมื่อไม่มี OIDC next โดยค่าเริ่มต้นคือ frontend origin แรกต่อด้วย `/login`
